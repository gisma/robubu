#' MakeUavRemoteControlledSurvey (mipla) is a tool to generate autonomous flight plans for an optimal picture retrieval with respect to DSM/DEM and orthophoto calculation. 
#' CreateUavf
#' 
...
#'   mipla (Make Uav Remote Controlled Survey) creates either intermediate flight control files for the dji phantom x UAVs or ready to upload control files for the 3DR Solo. 
...
#' 
#' fpdata<-mipla(surveyArea = c(50.80801,8.72993,50.80590,8.731153,50.80553,8.73472,50.8055,8.734),
#'               demFn = "~/dem.tif",
#'               heatMap = TRUE)
#'                         
#' ## view results
...
#' 
#' fpdata<-mipla(surveyArea = c(50.80801,8.72993,50.80590,8.731153,50.80553,8.73472,50.80709,8.734),
#'                    uavViewDir = 30,
#'                    demFn = "~/dem.tif",
#'                    heatMap = TRUE)
#'            
...
#' ## use it for planning
#' 
#' fpdata<-mipla(projectDir="~/uav/uav/test",
#'                    missionName = "50m",
#'                    surveyArea="~/uav.json", 
#'                    followSurface = TRUE, 
#'                    flightAltitude = 50,
                          ...
                          startLitchi=FALSE)
{
  ...
  levellog(logger, 'INFO', paste("estimated battery liftime  : ",maxFlightTime,      "  (min)      "))   
